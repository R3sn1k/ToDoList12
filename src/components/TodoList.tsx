...
function getDeadlineState(todo: SanityTodo) {
  if (!todo.dueDate || todo.completed) {
    return { expired: false, urgent: false, label: "Brez roka" }
  }
  ...
  if (diffHours <= 48) {
    return { expired: false, urgent: true, label: diffHours < 24 ? "Nujno danes" : "Nujno kmalu" }
  }

  return { expired: false, urgent: false, label: "Nacrtovano" }
}
...
export default function TodoList({
    ...
    try {
      const response = await fetch("/api/todos", {
        ...
        body: JSON.stringify({
          ...
          reminderEnabled: draftTask.dueDate ? draftTask.reminderEnabled : false,
  ...
  const saveEdit = async (id: string) => {
    ...
    try {
      ...
      await patchTodo(id, {
        ...
        reminderEnabled: editingTask.dueDate ? editingTask.reminderEnabled : false,
  ...
  const renderTodo = (todo: SanityTodo) => {
    ...
    return (
      <article
        key={todo._id}
        className={`rounded-[2rem] border p-5 shadow-[0_20px_60px_-44px_rgba(15,23,42,0.7)] ${todo.completed ? "border-emerald-200 bg-emerald-50/80" : deadline.expired ? "border-rose-200 bg-rose-50/80" : deadline.urgent ? "border-amber-200 bg-amber-50/80" : "border-slate-200 bg-white/90"}`}
      >
            ...
            {isEditing ? (
              <div className="space-y-3">
                <input
                  ...
                  onChange={(event) =>
                    setEditingTask((current) => ({ ...current, title: event.target.value }))
                  }
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <input
                    ...
                    onChange={(event) =>
                      setEditingTask((current) => ({ ...current, dueDate: event.target.value }))
                    }
                    className="rounded-[1.2rem] border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-slate-400"
                  />
                  <input
                    type="email"
                    value={editingTask.inviteEmail}
                    onChange={(event) =>
                      setEditingTask((current) => ({ ...current, inviteEmail: event.target.value }))
                    }
                    placeholder="Delegiraj uporabniku po e-posti"
                    className="rounded-[1.2rem] border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-slate-400"
                  />
                </div>
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <label className="flex items-center gap-3 text-sm text-slate-700">
                    <input
                      type="checkbox"
                      checked={editingTask.reminderEnabled && Boolean(editingTask.dueDate)}
                      ...
                      className="h-4 w-4 rounded border-slate-300 text-slate-950 focus:ring-slate-400 disabled:opacity-40"
                    />
                    Mail opomnik 1 dan prej
                  </label>
                </div>
                <div className="space-y-2 rounded-[1.4rem] border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        updateSubtasks("edit", (items) => [...items, { title: "", completed: false }])
                      }
                      className="rounded-full border border-slate-300 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-600"
                    >
                      Dodaj
                    </button>
                  </div>
                  {editingTask.subtasks.map((subtask, index) => (
                    <div key={`${todo._id}-${index}`} className="flex gap-2">
                      <input
                        value={subtask.title}
                        onChange={(event) =>
                          updateSubtasks("edit", (items) =>
                            items.map((item, itemIndex) =>
                              itemIndex === index ? { ...item, title: event.target.value } : item
                            )
                          )
                        }
                        className="flex-1 rounded-[1rem] border border-slate-200 bg-white px-3 py-2 outline-none transition focus:border-slate-400"
                      />
                      <button
                        type="button"
                        onClick={() =>
                        ...
                        className="rounded-full bg-rose-600 px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-white"
                      >
                        X
                      </button>
                    </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-3">
                      <h3
                        className={`text-2xl font-semibold tracking-tight ${todo.completed ? "text-emerald-900 line-through" : "text-slate-950"}`}
                      >
                        {todo.title}
                      </h3>
                      {todo.priority ? (
                ...
                {todo.subtasks.length > 0 ? (
                  <div className="mt-5 space-y-2 rounded-[1.5rem] border border-slate-200/80 bg-white/70 p-4">
                    ...
                    {todo.subtasks.map((subtask, index) => (
                      <label
                        key={subtask._key ?? `${todo._id}-${index}`}
                        className="flex items-center gap-3"
                      >
                        <input
                          type="checkbox"
                          checked={subtask.completed}
                          onChange={() => toggleSubtask(todo, subtask._key, index)}
                          className="h-4 w-4 rounded border-slate-300 text-slate-950 focus:ring-slate-400"
                        />
                        <span
                          className={`text-sm ${subtask.completed ? "text-slate-400 line-through" : "text-slate-700"}`}
                        >
                          {subtask.title}
                        </span>
                      </label>
          ...
          <div className="mt-5 space-y-3">
            {view.notifications.length === 0 ? (
              ...
              view.notifications.map((notification) => (
                <div key={notification._id} className="rounded-[1.5rem] bg-slate-50 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                    objavil {notification.createdBy.username}
                  </p>
                  <p className="mt-2 text-lg font-semibold text-slate-950">{notification.title}</p>
                  {notification.description ? (
                    <p className="mt-2 text-sm leading-6 text-slate-600">{notification.description}</p>
                  ) : null}
                  <p className="mt-2 text-sm text-slate-600">Rok: {formatDate(notification.dueDate)}</p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => respondToNotification(notification._id, "accepted")}
                    ...
                    <button
                      type="button"
                      onClick={() => respondToNotification(notification._id, "declined")}
        ...
        <div className="rounded-[2.2rem] border border-white/50 bg-white/75 p-6 shadow-[0_24px_80px_-48px_rgba(15,23,42,0.45)] backdrop-blur">
          ...
          <div className="mt-5 space-y-3">
            {view.invitations.length === 0 ? (
              <p className="rounded-[1.5rem] border border-dashed border-slate-300 px-4 py-5 text-sm text-slate-500">
                Trenutno ni odprtih povabil za prevzem nalog.
              </p>
            ) : (
              view.invitations.map((invitation) => (
                <div key={invitation._id} className="rounded-[1.5rem] bg-slate-50 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                    od {invitation.sender.username}
                  </p>
                  <p className="mt-2 text-lg font-semibold text-slate-950">{invitation.todo.title}</p>
                  <p className="mt-2 text-sm text-slate-600">Rok: {formatDate(invitation.todo.dueDate)}</p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      onClick={() => respondToInvitation(invitation._id, "declined")}
                      className="rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700"
                    >
                      Zavrni
                    </button>
          ...
          <div className="rounded-[2.2rem] border border-slate-900/10 bg-slate-950 p-6 text-white shadow-[0_30px_110px_-48px_rgba(15,23,42,0.78)]">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
              <article className="rounded-[1.7rem] border border-white/10 bg-white/6 p-5">
                <p className="text-sm text-slate-400">Aktivne naloge</p>
                <p className="mt-2 text-4xl font-semibold">{stats.active}</p>
              </article>
              <article className="rounded-[1.7rem] border border-white/10 bg-white/6 p-5">
                <p className="text-sm text-slate-400">Prednostne naloge</p>
                <p className="mt-2 text-4xl font-semibold">{stats.priority}</p>
              </article>
              <article className="rounded-[1.7rem] border border-white/10 bg-white/6 p-5">
                <p className="text-sm text-slate-400">Nujne naloge</p>
                <p className="mt-2 text-4xl font-semibold">{stats.urgent}</p>
              </article>
              <article className="rounded-[1.7rem] border border-white/10 bg-white/6 p-5">
                <p className="text-sm text-slate-400">Zgodovina</p>
                <p className="mt-2 text-4xl font-semibold">{stats.history}</p>
              </article>
            </div>
          </div>
          ...
          <div className="rounded-[2.2rem] border border-white/50 bg-white/75 p-6 shadow-[0_24px_80px_-48px_rgba(15,23,42,0.45)] backdrop-blur">
            ...
            <div className="mt-5 space-y-3">
              ...
              <textarea
                ...
                onChange={(event) =>
                  setDraftTask((current) => ({ ...current, description: event.target.value }))
                }
                rows={4}
                placeholder="Opis, opomba ali navodilo"
                className="w-full rounded-[1.2rem] border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-slate-400"
              />
              <div className="grid gap-3 md:grid-cols-2">
                <input
                  ...
                  onChange={(event) =>
                    setDraftTask((current) => ({ ...current, dueDate: event.target.value }))
                  }
                  className="rounded-[1.2rem] border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-slate-400"
                />
                <input
                  type="email"
                  value={draftTask.inviteEmail}
                  onChange={(event) =>
                    setDraftTask((current) => ({ ...current, inviteEmail: event.target.value }))
                  }
                  placeholder="Delegiraj uporabniku po e-posti"
                  className="rounded-[1.2rem] border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-slate-400"
                />
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <label className="flex items-center gap-3 text-sm text-slate-700">
                  <input
                    type="checkbox"
                    checked={draftTask.priority}
                    onChange={(event) =>
                      setDraftTask((current) => ({ ...current, priority: event.target.checked }))
                    }
                    className="h-4 w-4 rounded border-slate-300 text-slate-950 focus:ring-slate-400"
                  />
                  Dodaj med prednostne
                ...
                <label className="flex items-center gap-3 text-sm text-slate-700">
                  <input
                    type="checkbox"
                    checked={draftTask.reminderEnabled && Boolean(draftTask.dueDate)}
                    disabled={!draftTask.dueDate}
                    onChange={(event) =>
              ...
              <div className="space-y-2 rounded-[1.4rem] border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      updateSubtasks("draft", (items) => [...items, { title: "", completed: false }])
                    }
                    className="rounded-full border border-slate-300 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-600"
                  >
                    Dodaj
                ...
                {draftTask.subtasks.map((subtask, index) => (
                  <div key={`draft-${index}`} className="flex gap-2">
                    <input
                      value={subtask.title}
                      onChange={(event) =>
                        updateSubtasks("draft", (items) =>
                          items.map((item, itemIndex) =>
                            itemIndex === index ? { ...item, title: event.target.value } : item
                          )
                        )
              ...
              <button
                ...
                onClick={createTask}
                disabled={saving}
                className="w-full rounded-full bg-slate-950 px-5 py-3.5 text-sm font-semibold uppercase tracking-[0.24em] text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {saving ? "Shranjujem..." : "Ustvari nalogo"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-3 border-b border-slate-200/80 pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 className="mt-4 text-3xl font-semibold tracking-tight text-slate-950">
                  Najpomembnejse naloge
                </h2>
              </div>
              <p className="max-w-md text-sm leading-6 text-slate-600">
                Naloge lahko razporejas z vlecenjem po pomembnosti.
              </p>
            </div>
            <div className="mt-6 space-y-3">
              {view.priority.length === 0 ? (
                <p className="rounded-[1.5rem] border border-dashed border-slate-300 px-4 py-5 text-sm text-slate-500">
                  Oznaci nalogo kot prednostno in se bo pojavila tukaj.
                </p>
              ) : (
                view.priority.map((todo, index) => (
                  <div
                    key={todo._id}
                    draggable
                    onDragStart={() => setDraggedId(todo._id)}
                    onDragOver={(event) => event.preventDefault()}
                    onDrop={() => dropPriority(todo._id)}
                    className="flex items-center justify-between rounded-[1.6rem] border border-slate-200 bg-slate-50 px-4 py-4"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs uppercase tracking-[0.16em] text-slate-500">
                          {getDeadlineState(todo).label} - {formatDate(todo.dueDate)}
                        </p>
                      </div>
                    </div>
          ...
          <div className="rounded-[2.4rem] border border-white/50 bg-white/78 p-6 shadow-[0_26px_90px_-46px_rgba(15,23,42,0.5)] backdrop-blur">
            <div className="flex flex-col gap-3 border-b border-slate-200/80 pb-5 sm:flex-row sm:items-end sm:justify-between">
              ...
              <p className="max-w-md text-sm leading-6 text-slate-600">
                Naloga z bliznjim rokom postane nujna. Po preteku roka se nedokoncana premakne v zgodovino.
              </p>
            </div>
            <div className="mt-6 space-y-4">
              {view.active.length === 0 ? (
                <p className="rounded-[1.5rem] border border-dashed border-slate-300 px-4 py-8 text-center text-sm text-slate-500">
                  Ni aktivnih nalog. Ustvari novo ali sprejmi delegirano nalogo.
                </p>
              ) : (
                view.active.map(renderTodo)
              )}
            </div>
          </div>
          ...
          <div className="rounded-[2.4rem] border border-white/50 bg-white/78 p-6 shadow-[0_26px_90px_-46px_rgba(15,23,42,0.5)] backdrop-blur">
            <div className="flex flex-col gap-3 border-b border-slate-200/80 pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 className="mt-4 text-3xl font-semibold tracking-tight text-slate-950">
                  Zakljucene in zapadle naloge
                </h2>
              </div>
              <p className="max-w-md text-sm leading-6 text-slate-600">
                Tukaj so naloge, ki so dokoncane ali so jim roki ze potekli.
              </p>
            </div>
            <div className="mt-6 space-y-4">
              {view.history.length === 0 ? (
                <p className="rounded-[1.5rem] border border-dashed border-slate-300 px-4 py-8 text-center text-sm text-slate-500">
                  Zgodovina je se prazna.
                </p>
              ) : (
                view.history.map(renderTodo)
              )}
            </div>
          </div>
        </div>
